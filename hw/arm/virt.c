...
static void update_memory_regions(VirtMachineState *vms)
{
    ...
    /*
     * If valid iovas has only one entry, check the req size fits in and
     * can have the loader start < 4GB. This will make sure platforms with
     * no holes in mem will have the same mem model as before.
     */
    req_size = ram_size;
