...
static void
build_iort_rmr_nodes(GArray *table_data, GArray *smmuv3_accel,
                     size_t *smmu_offset, uint32_t *id)
{
    ...
    for (i = 0; i < smmuv3_accel->len; i++) {
        ...
        int bdf = accel->smmu_idmap.input_base;
        ...
        build_iort_id_mapping(table_data, bdf, accel->smmu_idmap.id_count,
                              smmu_offset[i], 1);
...
build_iort(GArray *table_data, BIOSLinker *linker, VirtMachineState *vms)
{
    ...
    if (vms->iommu == VIRT_IOMMU_SMMUV3_ACCEL) {
        build_iort_rmr_nodes(table_data, smmuv3_accel, smmu_offset, &id);
    }
