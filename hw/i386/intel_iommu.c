...
static int vtd_init_hwpt(VTDHwpt *hwpt, IOMMUFDDevice *idev, VTDPASIDEntry *pe)
{
...
static void vtd_destroy_hwpt(VTDHwpt *hwpt)
{
                                ...
                                VTDPASIDEntry *pe, VTDPASIDOp op)
{
    ...
    case VTD_PASID_BIND:
    {
         ...
         * accepts guest FLT under nesting. If pe->pgtt==PT, this should
         * return success, otherwise should return failure.
         */
        if (!vtd_pe_pgtt_is_flt(pe)) {
            return vtd_pe_pgtt_is_pt(pe) ? 0 : -1;
        }
        ...
        ret = vtd_init_hwpt(hwpt, idev, pe);
        if (ret) {
            break;
        }
    ...
    case VTD_PASID_UNBIND:
    {
        ...
        ret = iommu_device_detach_stage1(idev, hwpt->hwpt_id);
                                ...
                                VTDPASIDEntry *pe)
{
    ...
    if (pc_entry->cache_filled) {
        ...
        if (unlikely(ret)) {
            error_report("Failed to unbind stale table for pasid %u %d",
                         vtd_pasid_as->pasid, ret);
        }
                                  ...
                                  struct iommu_cache_invalidate_info *cache)
{
    ...
    if (iommufd_invalidate_cache(hwpt->iommufd, hwpt->hwpt_id, cache)) {
                                ...
                                IOMMUFDDevice *idev)
{
    ...
    if (iommu_device_get_info(idev, &type, sizeof(vtd), &vtd)) {
