...
bool write_cpustate_to_list(ARMCPU *cpu, bool kvm_sync)
{
    ...
    for (i = 0; i < cpu->cpreg_array_len; i++) {
        ...
        printf("%s: Shameer: kvm_sync %d cpu->cpreg_values[%d] 0x%lx\n", __func__, kvm_sync, i, cpu->cpreg_values[i]);
        ...
        if (kvm_sync) {
            if (is_id_reg(ri)) {
                printf("%s: Shameer: ID reg: set newval 0x%lx to cpu->cpreg_values[%d] 0x%lx\n", __func__, newval, i, cpu->cpreg_values[i]);
            } else {
                /*
